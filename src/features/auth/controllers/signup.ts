...
export class SignUp {
  ...
  public async create(req: Request, res: Response): Promise<void> {
    ...
    userDataForCache.profilePicture = `https://res.couldinary.com/${config.CLOUD_NAME}/image/upload/v${result.version}/${userObjectId}`;
    await userCache.saveToCache(`${userObjectId}`, uId, userDataForCache);

    // Add to DB
    const userOmittedData = omit(userDataForCache, ['uId', 'username', 'email', 'avatarColor', 'password']);
    authQueue.addAuthUserJob('addAuthUserToDB', { value: authData });
    userQueue.addUserJob('addUserToDB', { value: userOmittedData });
